...
public class RecordService {
	...
	public List<FileinfoUser> findFile(Record record){
		...
		DetachedCriteria criteria = DetachedCriteria.forClass(FileinfoUser.class,"files").createAlias("files.uploaduser", "user");

 		//設定條件
		try{ 
			// 使用者角色
			if (!record.getRole().equals("")) {
				criteria.add(Restrictions.eq("user.role", record.getRole()));
			...
			if (!record.getAccount().equals("")) {
				criteria.add(Restrictions.eq("user.account", record.getAccount()));
			...
			if (!record.getPriority().equals("")) {
				criteria.add(Restrictions.eq("user.priority", record.getPriority()));
			...
			if (record.getStartDate()!=null&&record.getEndDate()!=null) {
				criteria.add(Restrictions.between("uploaddate", record.getStartDate(), record.getEndDate()));
		...
		//查詢
		List<FileinfoUser> records = (List<FileinfoUser>) (dao.getHibernateTemplate()
