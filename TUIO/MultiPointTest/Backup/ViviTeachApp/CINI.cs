...
namespace CloudPaperApp
{
	...
	public class CINI{
        ...
        [DllImport("kernel32", SetLastError=true)] private static extern int GetPrivateProfileString(string lpAppName, string lpKeyName, string lpDefault, System.Text.StringBuilder lpReturnedString, int nSize, string lpFileName);
        ...
        public void CopyTo(string path)
        {
            try
            {
                System.IO.File.Copy(this.IniFile, path);
            }
            catch { }
        }
		...
		private string z_GetString(string pSection, string pKey, string pDefault){
            ...
            int i = GetPrivateProfileString(pSection, pKey, pDefault, temp, 255, ls_IniFilename);
